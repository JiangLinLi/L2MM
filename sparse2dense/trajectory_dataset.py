...
class Trajectory(Dataset):
    def __init__(self, all_data):
        ...
        self.src = []
        self.trg = []
        down_data = self.down_sample(all_data, self.interval)
        for x, y in zip(down_data, all_data):
            self.src.append(x)
    ...
    def down_data(self, data, interval):
        ...
        for line in data:
            if len(line) < interval:
                res.append(line)
            elif (len(line)-1) % interval == 0:
                index = np.arange(0, len(line), interval)
                line = np.arange(line)
                res.append(line[index]).tolist()
            elif (len(line) - 1) % interval != 0:
                index = np.arange(0, (int((len(line) - 1)/interval) + 1) * interval, interval)
                line = np.arange(line)
                tmp_res = np.append(line[index], line[-1])
                res.append(tmp_res.tolist())
        return res
